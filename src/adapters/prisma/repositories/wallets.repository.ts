...
export class WalletsRepository implements WalletPersistencePort {
  ...
  private toModel({ id, balance, userId, currency }: Prisma.WalletGetPayload<any>): WalletEntity {
    return new WalletEntity(id, new AmountEntity(Currency[currency], balance), userId);
  }

  private fromModel({ balance, userId }: WalletEntity): Prisma.WalletUncheckedCreateInput {
    const payload = { balance: balance.amount, currency: PrismaCurrency[balance.currency], userId };
